const SkillsSection = () => {
    return (
        <div className="bg-[#F7F7FB] text-center py-28">
            <p className="font-ibmplex font-semibold text-2xl mb-2">Skills</p>
            <p className="font-sourcesansae text-lg mb-14 text-[#808080]">Beberapa kemampuan saya</p>

            <div className="grid grid-cols-3 w-[930px] mx-auto gap-5">
                <div className="flex items-center bg-white py-5 px-7 rounded-lg shadow-xl">
                    <img src="/asset/logo/javascript.svg" className="rounded-full mr-5" alt="" />
                    <div className="text-start font-sourcesansae">
                        <p className="font-semibold text-lg">Javascript</p>
                        <p className="font-semibold text-sm text-[#A3A3A3]">Lanjutan</p>
                    </div>
                </div>
                <div className="flex items-center bg-white py-5 px-7 rounded-lg shadow-xl">
                    <img src="/asset/logo/javascript.svg" className="rounded-full mr-5" alt="" />
                    <div className="text-start font-sourcesansae">
                        <p className="font-semibold text-lg">Javascript</p>
                        <p className="font-semibold text-sm text-[#A3A3A3]">Lanjutan</p>
                    </div>
                </div>
                <div className="flex items-center bg-white py-5 px-7 rounded-lg shadow-xl">
                    <img src="/asset/logo/javascript.svg" className="rounded-full mr-5" alt="" />
                    <div className="text-start font-sourcesansae">
                        <p className="font-semibold text-lg">Javascript</p>
                        <p className="font-semibold text-sm text-[#A3A3A3]">Lanjutan</p>
                    </div>
                </div>
                <div className="flex items-center bg-white py-5 px-7 rounded-lg shadow-xl">
                    <img src="/asset/logo/javascript.svg" className="rounded-full mr-5" alt="" />
                    <div className="text-start font-sourcesansae">
                        <p className="font-semibold text-lg">Javascript</p>
                        <p className="font-semibold text-sm text-[#A3A3A3]">Lanjutan</p>
                    </div>
                </div>
                <div className="flex items-center bg-white py-5 px-7 rounded-lg shadow-xl">
                    <img src="/asset/logo/javascript.svg" className="rounded-full mr-5" alt="" />
                    <div className="text-start font-sourcesansae">
                        <p className="font-semibold text-lg">Javascript</p>
                        <p className="font-semibold text-sm text-[#A3A3A3]">Lanjutan</p>
                    </div>
                </div>
                <div className="flex items-center bg-white py-5 px-7 rounded-lg shadow-xl">
                    <img src="/asset/logo/javascript.svg" className="rounded-full mr-5" alt="" />
                    <div className="text-start font-sourcesansae">
                        <p className="font-semibold text-lg">Javascript</p>
                        <p className="font-semibold text-sm text-[#A3A3A3]">Lanjutan</p>
                    </div>
                </div>
                <div className="flex items-center bg-white py-5 px-7 rounded-lg shadow-xl">
                    <img src="/asset/logo/javascript.svg" className="rounded-full mr-5" alt="" />
                    <div className="text-start font-sourcesansae">
                        <p className="font-semibold text-lg">Javascript</p>
                        <p className="font-semibold text-sm text-[#A3A3A3]">Lanjutan</p>
                    </div>
                </div>
                <div className="flex items-center bg-white py-5 px-7 rounded-lg shadow-xl">
                    <img src="/asset/logo/javascript.svg" className="rounded-full mr-5" alt="" />
                    <div className="text-start font-sourcesansae">
                        <p className="font-semibold text-lg">Javascript</p>
                        <p className="font-semibold text-sm text-[#A3A3A3]">Lanjutan</p>
                    </div>
                </div>
                <div className="flex items-center bg-white py-5 px-7 rounded-lg shadow-xl">
                    <img src="/asset/logo/javascript.svg" className="rounded-full mr-5" alt="" />
                    <div className="text-start font-sourcesansae">
                        <p className="font-semibold text-lg">Javascript</p>
                        <p className="font-semibold text-sm text-[#A3A3A3]">Lanjutan</p>
                    </div>
                </div>
            </div>
        </div>
    )
}

export default SkillsSection